module.exports = {
    ...
    execute(client, message) {
        ...
        if (client.player.getQueue(message).paused) return message.channel.send(`${client.emotes.error} - A musica já esta parada!`);

        const success = client.player.pause(message);

        if (success) message.channel.send(`${client.emotes.success} - Song ${client.player.getQueue(message).playing.title} Música parada com sucesso !`);
    },
};
